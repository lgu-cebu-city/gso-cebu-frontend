...
@Component({
  selector: 'app-actual-po-selection-dialog',
  templateUrl: './actual-po-selection-dialog.component.html',
  styleUrls: ['./actual-po-selection-dialog.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class ActualPoSelectionDialogComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<ActualPoSelectionDialogComponent>,
  ) {
  ...
  loadData() {
    this.textFilterStr = "";
    this.httpRequest.getAllPurchaseOrderForIARActualByMonthYearWithDR(this.datepipe.transform(this.dateValue.value.toDate(), 'yyyy-MM-dd')).subscribe((result) => {
