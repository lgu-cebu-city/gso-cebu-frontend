...
import { InspectionAndAcceptanceReportActualComponent } from '../inspection-and-acceptance-report-actual.component';
import { IarListItemsComponent } from '../../acceptance-and-inspection-report/inspection-and-acceptance-report-list/iar-list-items/iar-list-items.component';
import { PrintPreviewIarComponent } from '../../acceptance-and-inspection-report/print-preview-iar/print-preview-iar.component';
...
@Component({
  selector: 'app-inspection-and-acceptance-report-actual-list',
  templateUrl: './inspection-and-acceptance-report-actual-list.component.html',
  styleUrls: ['./inspection-and-acceptance-report-actual-list.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class InspectionAndAcceptanceReportActualListComponent implements OnInit {
  ...
  loadData() {
    this.textFilterStr = "";
    this.isLoading = true;
    this.httpRequest.getAllAcceptanceAndInspectionReportActualByMonthYear(this.datepipe.transform(this.dateValue.value.toDate(), 'yyyy-MM-dd')).subscribe((result) => {
      if (result.statusCode == 200) {
  ...
  viewData(data: AcceptanceAndInspectionReportModel) {
    this.router.navigate(["/inspection-and-acceptance-report-actual", { id: data.id }]);
  }
  ...
  createFn() {
    const dialogRef = this.dialog.open(InspectionAndAcceptanceReportActualComponent, {panelClass: 'custom-dialog-container'},);
  ...
  editFn() {
    const dialogRef = this.dialog.open(InspectionAndAcceptanceReportActualComponent, {panelClass: 'custom-dialog-container', data: {
  ...
  deleteFn() {
    this.module.httpRequest.deleteAcceptanceAndInspectionReportActual(this.module.selectedIar.id).subscribe((result) => {
      if (result.statusCode == 200) {
