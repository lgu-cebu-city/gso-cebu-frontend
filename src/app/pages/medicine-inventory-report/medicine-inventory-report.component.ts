...
@Component({
  selector: 'app-medicine-inventory-report',
  templateUrl: './medicine-inventory-report.component.html',
  styleUrls: ['./medicine-inventory-report.component.css']
})
export class MedicineInventoryReportComponent implements OnInit {
  env = environment;
  ...
  loadData() {
    ...
    if (this.authService.getTypeId() != environment.gsoDeptId.toString()) {
      _deptId = this.authService.getTypeId();
    }
    this.httpRequest.getAllInventoryReportMedicine(_deptId).subscribe((result) => {
  ...
  printSSMI(){
    this.module.httpRequest.getSSMIReport(2022).subscribe((result) => {
      if (result.statusCode == 200) {
