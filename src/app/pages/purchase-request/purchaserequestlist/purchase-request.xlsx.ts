...
export class PurchaseRequestXLSX {
  ...
  public generateXLSXFormat(data: PurchaseRequestModel) {
    ...
    worksheet.getCell('E4').font = { bold: true, underline: true };
    ...
    worksheet.getCell('E5').font = { bold: true, underline: true };
    ...
    for (var i = 0; i <= itemLen; i++) {
      ...
      if (data.items[i]) {
        temp = [
          ...
          data.items[i].quantity.toLocaleString('en-US', { maximumFractionDigits: 2 }),
        ...
        if (['-','0'].indexOf(data.items[i].id) < 0) {
      ...
      if (data?.items[i]?.id == '-') {
        ...
        worksheet.getCell('E' + (currRow)).value = data.items[i].description;
        ...
        worksheet.getCell('F' + currRow).font = { bold: true };
      } else if (data?.items[i]?.id == '0') {
        worksheet.mergeCells('A' + currRow + ':G' + currRow);
        worksheet.getCell('E' + (currRow)).value = data.items[i].description;
        worksheet.getCell('A' + currRow).font = { bold: true };
      } else {
        worksheet.mergeCells('F' + currRow + ':G' + currRow);
