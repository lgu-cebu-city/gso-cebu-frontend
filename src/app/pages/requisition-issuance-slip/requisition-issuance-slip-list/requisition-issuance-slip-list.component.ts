...
import { RequisitionIssuanceSlipComponent } from '../requisition-issuance-slip.component';
``
@Component({
  selector: 'app-requisition-issuance-slip-list',
  templateUrl: './requisition-issuance-slip-list.component.html',
  styleUrls: ['./requisition-issuance-slip-list.component.css']
})
export class RequisitionIssuanceSlipListComponent implements OnInit {
  envFirstLoad = environment.firstLoad;
  displayedColumns: string[] = ['transactionNo', 'transactionDate', 'saiNo', 'saiDate', 'division', 'departmentName', 'issuanceType', 'purpose', 'action'];
  displayedColumnsNoAction: string[] = ['transactionNo', 'transactionDate', 'saiNo', 'saiDate', 'division', 'departmentName', 'issuanceType', 'purpose'];
  ...
  loadData() {
    if (this.authService.getTypeId() == environment.gsoDeptId.toString()) {
      this.httpRequest.getAllIssuance().subscribe((result) => {
      ...
      this.httpRequest.getAllIssuanceByDepartment(this.authService.getTypeId()).subscribe((result) => {
        if (result.statusCode == 200) {
          this.risList = result.data;
          this.dataSource = new MatTableDataSource<RequisitionAndIssuanceModel>(this.risList);
          this.dataSource.paginator = this.paginator;
  ...
  viewData(data: any) {
    this.router.navigate(["/issuance-slip", { id: data.id }]);
  }
  ...
  createFn() {
    const dialogRef = this.dialog.open(RequisitionIssuanceSlipComponent, {panelClass: 'custom-dialog-container'},);
  ...
  editFn() {
    const dialogRef = this.dialog.open(RequisitionIssuanceSlipComponent, {panelClass: 'custom-dialog-container', data: {
